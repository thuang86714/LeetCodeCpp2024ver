...
class Solution {
public:
    std::vector<int> findAllPeople(int n, std::vector<std::vector<int>>& A, int firstPerson) {
        std::sort(begin(A), end(A), [](auto &a, auto &b) { return a[2] < b[2]; }); // Sort the meetings in ascending order of meeting time
        UnionFind uf(n);
        ...
        for (int i = 0; i < n; ++i) {
            if (uf.connected(0, i)){
                ans.push_back(i);
            } // Push all the persons who are connected with person 0 into answer array
        }
        return ans;
    }
};